...
public class TransformerModelInferenceService implements ModelInferenceService {

    private final ZooModel<Inference, Float> model;
    ClusterSize clusterSize;

    public TransformerModelInferenceService(ClusterSize clusterSize) throws ModelNotFoundException, MalformedModelException, IOException {
        this.clusterSize = clusterSize;
        String modelName;
        if (clusterSize == ClusterSize.THREE){
            modelName = "TRANSFORMER-3";
        }else{
            modelName = "TRANSFORMER-6";
        }
